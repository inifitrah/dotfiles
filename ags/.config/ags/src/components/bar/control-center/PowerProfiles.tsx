  ...
  return (
    ...
    >
      <box hexpand spacing={6}>
        <label className="big-icon"></label>
        <box vertical hexpand>
          <label className="title" xalign={0}>
            {bind(powerprofiles, "activeProfile").as(fmtProfile)}
          </label>
          <label className="value" xalign={0}>
            {bind(powerprofiles, "activeProfile").as((p) =>
              fmtProfile(nextProfile(p))
            )}
          </label>
        </box>
      </box>
