import options from "../dropdown/shared/menu/components/options";
...
function getFocusedHyprlandMonitor(): AstalHyprland.Monitor | undefined {
  ...
  return allMonitors.find(
    (monitor) => monitor.id === hyprlandService.focusedMonitor.id
  );
}
 ...
 */
function applyMonitorScaling(
  width: number,
  height: number,
  monitorScaling: number
): MonitorScaling {
  ...
  if (userScalingPriority === "both") {
    ...
    /^\d+(\.\d+)?$/.test(gdkEnvScale) &&
    userScalingPriority === "gdk"
  ) {
    const gdkScaleValue = parseFloat(gdkEnvScale);
    adjustedWidth /= gdkScaleValue;
    adjustedHeight /= gdkScaleValue;
...
export const calculateMenuPosition = async (
  ...
  try {
    ...
    const dropdownWidth =
      dropdownEventBox.get_child()?.get_allocation().width ?? 0;
    const dropdownHeight =
      dropdownEventBox.get_child()?.get_allocation().height ?? 0;
