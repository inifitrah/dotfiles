...
uniform float progress;      // 0 -> 1 selama open
uniform float seed;          // random per-window 0..1
uniform vec2 surface_size;   // ukuran window (untuk scanline)
...
void main() {
    ...
    // HyprWindowShade: v_texcoord sudah 0-1, jadi langsung texture(tex, uv)
    vec4 color;
    ...
    float scanline = 1.0 - sin(uv.y * surface_size.y * 3.14159) * 0.06 * intensity;
    color.rgb *= scanline;

    float alpha = smoothstep(0.0, 0.15, p);
    // premultiply + alpha reveal (wajib untuk HyprWindowShade)
    color.rgb *= alpha;
    color.a *= alpha;
